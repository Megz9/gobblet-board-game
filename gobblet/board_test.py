...
class Board_Test:
    def __init__(self, win):
        win.fill(BACKGROUND)
        self.board_test = [[0] * COLS for _ in range(ROWS)]
    ...
    def draw_initial_board(self, win):
        for row in range(ROWS):
            for col in range(COLS):
                ...
                self.board_test[row][col] = Tile(x, y)
    ...
    def select(self, win):
        ...
        if tile != None and self.selected_tile != None:
            ...
            Board_Test.highlight_tile(win, tile)
    ...
    def get_tile_from_pos(self, pos):
        ...
        if mouse_x > BOARD_START_X and mouse_x < BOARD_START_X + BOARD_SIZE:
            ...
            print(row, col)
            return self.board_test[row][col]
    ...
    def draw_board(self, win):
        for row in range(ROWS):
            for col in range(COLS):
                ...
                pygame.draw.rect(win, BEIGE if (row + col) % 2 == 0 else BROWN,
                                 (x, y, SQUARE_SIZE, SQUARE_SIZE))
                self.draw_tile(win, self.board_test[row][col])
